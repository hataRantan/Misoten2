...
public class MyPlayerMoveAction : MonoBehaviour
{
    ...
    public void MoveMode(int _moveNum, Vector2 _input)
    {
        ...
        switch (myMoveType)
        {
            case MyMoveType.JUMP:
                if (!jumpFlg)
                {
                    StartCoroutine(MoveJumpCoroutine(endPos.position, flightTime, speedRate, gravityValue));
                }
                break;
    ...
    private IEnumerator MoveJumpCoroutine(Vector3 endPos, float flightTime, float speedRate, float gravityValue)
    {
        jumpFlg = true;
